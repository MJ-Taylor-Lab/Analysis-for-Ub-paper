...
tryCatch({
  #Calculate when first spot appears in cell, first frame could be either MyD88 or the other protein
  ExpDataLand <-
    ExpData %>%
  ...
  ExpData <-
    ExpData %>%
    # Remove frames before cell lands, keep only frames detected by TrackMate
    filter(
      FRAMES_ADJUSTED >= 0
    ) %>%
    #Sort by track
    group_by(TRACK_ID) %>%
    #FRAME_LAND & TIME_LAND reset the landing frame and time to 0 for that cell 
    mutate(
      FRAME_LAND = FRAME - LANDED,
      FPS = lp.FPS,
      TIME_LAND = FRAME_LAND/FPS,
    ...
    mutate(
      ...
      MAX_INTENSITY_FRAME = ifelse(MAX_NORMALIZED_INTENSITY == NORMALIZED_INTENSITY, 1, 0),
      #For sorting assembled and abortives
      MAX_INTENSITY_CAT = ifelse(MAX_NORMALIZED_INTENSITY >= MAX_INTENSITY_THRESHOLD, 1, 0),
      #To tell if current frame meets max intensity threshold
      #FRAME_MAX_INTENSITY_CAT = ifelse(NORMALIZED_INTENSITY >= MAX_INTENSITY_THRESHOLD, 1, 0),
      ...
      #Max minus intensity at the end
      #MAX_TO_END_INTENSITY =  as.numeric(MAX_NORMALIZED_INTENSITY) - as.numeric(ENDING_NORMALIZED_INTENSITY),
      #Change max intensity frame identifier to actual frame number
      #MAX_INTENSITY_FRAME = ifelse(MAX_INTENSITY_FRAME == 1, FRAMES_ADJUSTED, NA),
      ...
      #Separates de-novo processes based on the intensity of the starting frame
