...
TracksSummary <-
  ProteinData %>%
  mutate(
    MAX_INTENSITY_CAT = "All"
  )

TracksSummary <- rbind(TracksSummary, ProteinData)

MAX_INTENSITY_CATS <- c("All", sort(unique(ProteinData$MAX_INTENSITY_CAT)))

TracksSummary <-
  TracksSummary %>%
  mutate(
    MAX_INTENSITY_CAT = factor(MAX_INTENSITY_CAT, levels = MAX_INTENSITY_CATS)
  )
