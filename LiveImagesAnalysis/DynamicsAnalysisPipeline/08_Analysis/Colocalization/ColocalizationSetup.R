...
if(GENERATE_NEW_TABLE == T) {
  ...
  if(exists("ColocalizationCombined")) {} else {
    setwd(COLOCALIZATION_DIRECTORY)
  ...
  tryCatch({
    ...
    ProteinFx <- function(ProteinX) {
      tryCatch({
        ...
        #Percent colocalization by MaxInt
        #Table
        setwd(COLOCALIZATION_SCRIPTS)
        ...
        setwd(COLOCALIZATION_SCRIPTS)
        print("Running script Mean_Colocalization Cat_MaxIntCat By_Replicate Violin.R")
        source("Mean_Colocalization Cat_MaxIntCat By_Replicate Violin.R", local = T)
